<div class="row">
	<p>This inject has <strong><?= count($hints); ?> hint<?= count($hints) > 1 ? 's' : ''; ?> available</strong>.</p>
		...
		<tbody>
			...
			<tr>
				<td width="80%" colspan="<?= $hint['Hint']['unlocked'] ? '2' : '1'; ?>">
					...
					<p>Hint will be available after <?= $hint['Hint']['time_wait']; ?> seconds</p>
					<?php endif; ?>
